var AreaModel=function(){this.trash=[];this.calcMostRect=function(){for(var b=0;b<this.trash.length;b++)this.trash[b].calcMostRect(this)};this.isBlankDay=function(b){var c=[this.center.startDate,this.center.endDate];return c[0].getTime()<=b.getTime()&&b.getTime()<=c[1].getTime()?!0:!1};this.setCenter=function(b){for(var c in b)this.centerName==b[c].name&&(this.center=b[c])};this.sortTrash=function(){this.trash.sort(function(b,c){var f=b.mostRecent.getTime(),d=c.mostRecent.getTime();return f<d?-1:
f>d?1:0})}},Description=function(){},TrashModel=function(b,c){function f(b){for(var a=0;a<m.length;a++)if(m[a]==b)return a;return-1}this.dayCell=c.split(" ");this.label=b;var d="",e;for(e in this.dayCell)d=1==this.dayCell[e].length?d+("\u6bce\u9031"+this.dayCell[e]+"\u66dc\u65e5 "):d+("\u7b2c"+this.dayCell[e].charAt(1)+this.dayCell[e].charAt(0)+"\u66dc\u65e5 ");this.dayLabel=d;this.getDateLabel=function(){var b=this.mostRecent.getFullYear()+"/"+(1+this.mostRecent.getMonth())+"/"+this.mostRecent.getDate();
return this.dayLabel+" "+b};var m="\u65e5\u6708\u706b\u6c34\u6728\u91d1\u571f".split("");this.calcMostRect=function(b){for(var a=this.dayCell,n=[],p=new Date,c=4;15>=c;c++)for(var e in a)for(var d=!1,h=0;5>h;h++){var k=new Date(2013,c-1,1),g=new Date(k);1==a[e].length?(g.setTime(k.getTime()+864E5*((7+f(a[e])-k.getDay())%7)+6048E5*h),g.getMonth()+1==c%12&&(n.push(g),null==this.mostRecent&&g.getTime()>p.getTime()&&(this.mostRecent=g))):(g.setTime(k.getTime()+864E5*((7+f(a[e].charAt(0))-k.getDay())%
7)+6048E5*h),b.isBlankDay(g)&&(d=!0),d&&g.setTime(g.getTime()+6048E5),h==a[e].charAt(1)-1&&(n.push(g),null==this.mostRecent&&g.getTime()>p.getTime()&&(this.mostRecent=g)))}this.dayList=n};this.getDayList=function(){var b="<ul>",a;for(a in this.data_list)var n=data_list[a],b=b+("<li>"+n.getFullYear()+"/"+(n.getMonth()+1)+"/"+n.getDate()+"</li>");return b+"</ul>"}},CenterModel=function(b){function c(b,c){var e=b[c].split("/");return new Date(e[0],e[1]-1,e[2])}this.name=b[0];this.startDate=c(b,1);this.endDate=
c(b,2)};function get_selected_area_name(){return localStorage.getItem("selected_area_name")}function set_selected_area_name(b){localStorage.setItem("selected_area_name",b)}
